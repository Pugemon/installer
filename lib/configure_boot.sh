...
syslinux_config() {
    ...
    if "$UEFI" ; then
        ...
        cp "${trap_directory}"/boot/loader/syslinux/syslinux_efi.cfg ${ARCH}${esp_mnt}/EFI/syslinux/syslinux.cfg
        cp "${trap_directory}"/boot/splash.png ${ARCH}${esp_mnt}/EFI/syslinux
        ...
        cp "${trap_directory}"/boot/loader/syslinux/syslinux.cfg "$ARCH"/boot/syslinux/
        cp "${trap_directory}"/boot/splash.png "$ARCH"/boot/syslinux/) &> /dev/null &
